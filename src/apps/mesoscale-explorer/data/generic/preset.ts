...
import { ModelFromTrajectory, ShapeFromPly, LinesFromKin, /*MeshFromKin,*/ TrajectoryFromGRO, TrajectoryFromMOL, TrajectoryFromMOL2, TrajectoryFromMmCif, TrajectoryFromPDB, TrajectoryFromSDF, TrajectoryFromXYZ } from '../../../../mol-plugin-state/transforms/model';
            ...
            for (const ent of manifest.entities) {
                ...
                if (['gro', 'cif', 'mmcif', 'mcif', 'bcif', 'pdb', 'ent', 'xyz', 'mol', 'sdf', 'sd', 'mol2'].includes(info.ext)) {
                  ...
                  if (['kin'].includes(info.ext)) {
                    ...
                    build = build
                      .apply(ParseKin)
                      .apply(LinesFromKin, { label, transforms })
